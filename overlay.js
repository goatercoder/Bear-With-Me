 ...
 * every web page, so he bleeds right there on YouTube in front of you. The
 * Stanford Tree walks in from the left and beats him while you're there. */
(function () {
  'use strict';
  ...
  function mount() {
    ...
    host.style.cssText = 'position:fixed;top:8px;right:8px;z-index:2147483647;height:' + H + 'px;pointer-events:none;';
    const root = host.attachShadow({ mode: 'closed' });
    const style = document.createElement('style');
    style.textContent = `
      .box { position: relative; height: ${H}px; pointer-events: auto; cursor: default; }
  ...
  function describe(s) {
    ...
    return k === 'bad' ? `${pct}% · the Tree is killing Oski. Leave this site.` : s.health < 100 ? `${pct}% · healing` : 'Oski is fine';
  }
